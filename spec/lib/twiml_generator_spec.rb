describe TwimlGenerator do
  ...
  describe '.generate_wait' do
    it 'generates twiml with say and play nodes' do
      ...
      expect(nodes[0].content).to eq('Thank you for calling. Please wait in line for a few seconds. An agent will be with you shortly.')
      expect(nodes[1].name).to eq('Play')
      expect(nodes[1].content).to eq('http://twimlets.com/holdmusic?Bucket=com.twilio.music.classical')
    end
  end
end
